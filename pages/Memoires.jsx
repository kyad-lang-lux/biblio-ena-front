...
const Memoires = () => {
  ...
  useEffect(() => {
    ...
    if (listRef.current) {
      ...
      cards.forEach(card => {
        ...
        const student = card.querySelector('.meta-item:nth-child(1)')?.textContent.toLowerCase() || '';
        const deweyTag = card.querySelector('.category-tag')?.textContent || '';

        const matchesSearch = title.includes(term) || student.includes(term);
  ...
  return (
    <div className="memoires-page monographies-page">
      <section className="document-list-header">
        ...
        <div className="title-row">
          <h1>Mémoires</h1>
          <span className="doc-count-total">Travaux de recherche</span> 
        </div>
